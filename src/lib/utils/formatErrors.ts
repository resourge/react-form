...
function addErrorToFormErrors<T extends Record<string, any>>({
	formErrors, isChildError = false, path, prev, validationError
}: ErrorToFormError<T>): PrevFormError<T> {
...
export const formatErrors = <T extends Record<string, any>>(
	...
	.reduce<FormErrors<T>>((val, value) => {
		...
		forEachPossibleKey(value.path, (possibilityKey) => {
			prev = addErrorToFormErrors({
				formErrors: val, 
				isChildError: value.path !== possibilityKey, 
				path: possibilityKey,
				prev,
